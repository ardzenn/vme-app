document.addEventListener('DOMContentLoaded', () => {
    ...
    async function viewOrder(orderId) {
        try {
            ...
            if(invoiceContainer) {
                invoiceContainer.innerHTML = '<div class="alert alert-danger">Could not load order details.</div>';
            }
            orderDetailModal.show();
        }
    }
    ...
    orderDetailModalEl.addEventListener('shown.bs.modal', function (event) {
        ...
        if (order.attachment) {
            ...
            attachmentSection.innerHTML = `<p class="text-muted">No attachment found.</p><form action="/orders/${order._id}/attach" method="POST" enctype="multipart/form-data"><div class="input-group"><input type="file" class="form-control" name="attachment" required><button class="btn btn-outline-secondary" type="submit">Upload</button></div></form>`;
        }
    ...
    function appendMessage(msg, currentUserId) {
        ...
        let attachmentHTML = '';
        if (msg.attachment) {
            if (/\.(jpg|jpeg|png|gif)$/i.test(msg.attachment)) {
        ...
        const messageHTML = `<div class="chat-message ${messageClass}"><img src="${profilePic}" class="chat-profile-pic"><div class="chat-bubble"><div class="username">${msg.user.firstName}</div>${msg.text ? `<div class="chat-text">${msg.text}</div>` : ''}${attachmentHTML}<div class="timestamp">${timestamp}</div></div></div>`;
